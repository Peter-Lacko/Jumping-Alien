...
public class ProgramTest {
	...
	public void setUpMutableFixture(){
		...
		globalVariables2.put("p", new Object(null));
		globalVariables2.put("y", new DoubleType(0.0));
		Expression<Object> variable2aa = new ExpressionVariableGameObject(loc, "p");
		Expression<Object> variable2ab = new ExpressionVariableGameObject(loc, "p");
		Expression<Object> variable2ac = new ExpressionVariableGameObject(loc, "p");
		Expression<DoubleType> variable2b = new ExpressionVariableDouble(loc, "y");
		Expression<DoubleType> pX = new GetX(variable2aa, loc);
		Expression<DoubleType> selfX = new GetX(new SelfObject(loc),loc);
		Expression<Bool> whereExpression = new SmallerThan(pX, selfX, loc);
		Statement subsubStatement2a = new Print<DoubleType>(new GetHp(variable2ab, loc), loc);
		Expression<DoubleType> getY = new GetY(variable2ac, loc);
		Statement subsubStatement2b = new Assignment<DoubleType>("y", getY, loc, new DoubleType(0.0));
		...
		Statement subsubStatement2d = new Wait(new ExpressionBasic<DoubleType>(new DoubleType(1.0), loc),loc);
	...
	public void failedProgramTest(){
		...
		worldStandard.advanceTime(0.01);
		assertFalse(buzam1.isMovingRight());
		//If the program was correct, Buzam1 would be moving in one of the next 2 times
		worldStandard.advanceTime(0.005);
		assertFalse(buzam1.isMovingRight());
		worldStandard.advanceTime(0.005);
		assertFalse(buzam1.isMovingRight());
	}
	...
	public void workingProgramTest(){
		worldStandard.advanceTime(0.01);
		//(Buzam is ducked)
		assertEquals(1.0, buzam2.getMaxHorizontalVelocity(), Util.DEFAULT_EPSILON);
		worldStandard.advanceTime(0.007);
		//(Buzam is no longer ducked)
		assertEquals(3.0, buzam2.getMaxHorizontalVelocity(), Util.DEFAULT_EPSILON);
		for (int i = 1; i<= 10; i++){
			worldStandard.advanceTime(0.1);
		}
		assertTrue(buzam2.isJumping());
		worldStandard.advanceTime(0.003);
		assertFalse(buzam2.isJumping());
		worldStandard.advanceTime(0.001);
		worldStandard.advanceTime(0.01);
		//(Buzam is ducked again)
		assertEquals(1.0, buzam2.getMaxHorizontalVelocity(), Util.DEFAULT_EPSILON);
		for (int i = 1; i <= 5; i++)
			worldStandard.advanceTime(0.1);
		assertTrue(buzam2.isJumping());
	...
	public void notWellFormed_ActionInForEach(){
		Statement action = new StartJump(loc);
		Statement whileLoop = new WhileLoop(new ExpressionBasic<Bool>(new Bool(true), loc),action,loc);
		Statement forEach = new ForEachLoop("x", Kind.ANY, null, null, null, loc, whileLoop);
		Map<String, Type<?>> map = new HashMap<>();
		map.put("x", new Object(null));
		Program prog = new Program(forEach, map);
		assertFalse(prog.isWellFormed());
	}
