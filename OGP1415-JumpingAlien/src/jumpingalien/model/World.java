...
public class World {
	 ...
	 * 
	 * @param mazub
	 * @effect	if any of Mazub's edges are in the same tile as the target tile, then the character has won.
	 * 			|let	pos = mazub.getIntPosition()
	 * 			|		width = mazub.getSprite().getWidth()
	 * 			|		height = mazub.getSprite().getHeight()
	 * 			|if
	 * 			|	(for some position in {pos[0]..pos[0]+width-1, pos[1]}, 
	 * 			|						{pos[0]..pos[0]+width-1, pos[1]+height-1},
	 * 			|						{pos[0], pos[1]..pos[1]+height-1},
	 * 			|						{pos[0]+width-1, pos[1]..pos[1]+height-1}
	 * 			|		getTilePosition(position) == getTargetTile())
	...
	//		int[] mazubPos1 = {mazub.getIntPositionAt(1), mazub.getIntPositionAt(2)};
	//		int[] mazubPos2 = {mazub.getIntPositionAt(1) + mazub.getSprite().getWidth(), mazub.getIntPositionAt(2)};
	//		mazubPos1 = getPixelOfTileContaining(mazubPos1[0], mazubPos1[1]);
	//		mazubPos2 = getPixelOfTileContaining(mazubPos2[0], mazubPos2[1]);
	//		int [] target = getBottomLeftPixelOfTile(getTargetTile()[0], getTargetTile()[1]);
	////		if ((mazubPosition[0] == getTargetTile()[0]) && (mazubPosition[1] == getTargetTile()[1])){
	//		if (((mazubPos1[0] == target[0]) && (mazubPos1[1] == target[1])) 
	//				|| ((mazubPos2[0] == target[0]) && (mazubPos2[1] == target[1]))){
	 ...
	 */
	public boolean hasAsObject(@Raw Characters object){
	 ...
	 */
	public void addObjectAt(@Raw Characters object, int index) throws IllegalArgumentException{
	...
	public List<GameObject> getAllObjectsAndTiles(){
	...
	public Collection<Buzam> getAllBuzams(){
		Collection<Buzam> buzams = new LinkedList<Buzam>();
		for (Characters object : getAllObjects()){
			if (object instanceof Buzam)
				buzams.add((Buzam)object);
		}
		return buzams;
	}
